...
**Done today — async page 500 FIXED (awaited failed useFetch no longer 500s SSR) + prod re-measured:**
- [x] **Root cause:** `vesk-doc/app/components/VersionBadge.vsk` did `const latest = await useFetch.json('https://registry.npmjs.org/@vesk/compiler/latest')` — on this box Node's egress to the registry can time out (10s connect), the `await` rejected, and the badge sits in the layout so **every page returned HTTP 500** (log: `vesk ssr error: Request timed out after 10000ms`, `resource.ts:49`). Warm TTFB was ~90–110ms but masked by the 500s; cold first hit ~21s.
- [x] **Framework fix (`packages/runtime/src/resource.ts` `toData()`):** SSR fail-open — an awaited resource whose *fetch* failed (`HttpError` / `TimeoutError` / network `TypeError`) resolves `undefined` instead of rejecting, so the developer's null/fallback branch renders and the page stops 500ing. Non-fetch code errors still reject so `error.vsk` still surfaces real bugs; client keeps rejecting for `try/catch`. New `isFetchError()` classifier.
- [x] **Badge fix (`VersionBadge.vsk`):** dropped `async`/`await`, now a reactive statement-mode guard (`if (latest.loading || latest.error) return`) rendering `<span>v{latest.data!.version}</span>` only on success.
- [x] **Tests + docs:** 4 new SSR fail-open tests in `resource.test.ts` (HttpError resolves undefined, TimeoutError resolves undefined, non-fetch error still throws, success still resolves data) — full runtime suite green (all `*.test.ts` incl. 342 hmr). Cleanup of the stale "store `slot-props.test.ts` 500-char-truncation" confusion — none. docs.md + llms.txt updated (fail-open semantic documented).
- [x] **Verified live:** vesk-doc re-refreshed to `0.2.31-ci.1789956051923` tarballs carrying the runtime fix, rebuilt, fresh `vesk start -p 4000` → `/` 200, `/docs` 200, `/docs/lsp-editors` 200, `/docs/ai-skills` 200, badge renders `v0.2.31` (registry reachable today), **0** SSR timeout/500s. Cold TTFB ~5.7s, warm ~0.5–0.9s (was 500-everywhere, cold ~21s). `npm run typecheck` clean; vesk-doc typecheck clean. Note: a leftover test-app `vesk start -p 4000` squatter was answering `/docs/*` as 404 mid-measurement — killed both test-app servers first (`/docs/quickstart` is a fake slug, correct 404). Commit `c00ae9e` (fix(runtime): awaited failed useFetch fails open at SSR; VersionBadge falls back).

**Done today — skills installer + docs, vesk-doc dep refresh (curl|bash era):**
- [x] `scripts/install-skills.sh` (curl `| bash` installer mirroring `install.sh`) + `/docs/ai-skills` page — commit `69ab79c`. vesk-doc deps refreshed to `0.2.31-ci.1789932358219` tarballs + `.cursor`/`.windsurf` skill READMEs — commit `906d79a`. Full public-API surface: `/docs/ai-skills`, `/docs/editor`, `/docs/install`.
