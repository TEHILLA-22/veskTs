...
console.log('\n=== useFetch — SSR awaited-fetch fail-open (async page 500 fix) ===');

it('SSR: awaited HttpError fetch fails open (resolves undefined) instead of throwing', async () => {
  cleanupGlobals();
  (globalThis as any).__vsk_ssr = true;
  (globalThis as any).__vsk_ssr_token = 'ssr-failopen-http';
  const mock = mockFetch(() => jsonResponse({}, 503));
  const res = useFetch<{ version: string }>('/api/ssr-503');
  await waitFor(() => res.error !== null);
  ...
  expect(awaited).toBe(undefined);
  // .error stays readable for reactive branches
  expect((res.error as HttpError).status).toBe(503);
  mock.restore();
  cleanupGlobals();
});

it('SSR: awaited TimeoutError fetch fails open instead of throwing', async () => {
  cleanupGlobals();
  (globalThis as any).__vsk_ssr = true;
  (globalThis as any).__vsk_ssr_token = 'ssr-failopen-timeout';
  const mock = mockFetch(() => new Promise<Response>(() => {}));
  const res = useFetch('/api/ssr-slow', { timeout: 20 });
  await waitFor(() => res.error !== null);
  expect(res.error instanceof TimeoutError).toBe(true);
  let threw: unknown;
  try {
...
it('SSR: awaited non-fetch error still throws (genuine code bugs surface)', async () => {
  cleanupGlobals();
  (globalThis as any).__vsk_ssr = true;
  (globalThis as any).__vsk_ssr_token = 'ssr-failopen-bug';
  const res = createResource<string>(() => Promise.reject(new Error('logic bug')), 'ssr-bug-key');
  await waitFor(() => res.error !== null);
  let threw: unknown;
  try {
    await res;
  ...
  expect((threw as Error).message).toBe('logic bug');
  cleanupGlobals();
});

it('SSR: awaited successful fetch still resolves with data', async () => {
  cleanupGlobals();
  (globalThis as any).__vsk_ssr = true;
  (globalThis as any).__vsk_ssr_token = 'ssr-failopen-ok';
  const mock = mockFetch(() => jsonResponse({ version: '1.2.3' }));
  const res = useFetch<{ version: string }>('/api/ssr-ok');
  await waitFor(() => res.loading === false);
  expect(await res).toEqual({ version: '1.2.3' });
  mock.restore();
  cleanupGlobals();
});
