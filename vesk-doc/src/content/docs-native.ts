...
export const pages: { slug: string; title: string; description: string; group: string; blocks: Block[] }[] = [
  {
    ...
    blocks: [
      {
        kind: "p",
        text: "Say you built an app as `.vsk` for the web last week — the marketing pages, the dashboard, the checkout. Vesk Native is how that same source ships as an Android app: `@vesk/native-compiler` translates `.vsk` files (Markup + Tailwind + scripts) into native Kotlin/Compose, not a WebView wrapped around a web build. The compiler walks the same `@vesk/compiler` IR the web compiler produces, so the markup you wrote for the browser renders as real Android views with real platform APIs. No Android Studio needed: every generated artifact — Kotlin source, manifest permissions, assets — is derived from what the app's `.vsk` pages actually use, never copied wholesale.",
      },
      { kind: "h2", text: "Packages" },
      {
        ...
        head: ["Package", "Role"],
        rows: [
          ["@vesk/native", "App-facing API: `defineConfig` and the `VeskConfig`/`VeskSigning`/`VeskBundle`/`VeskThemeMode` types for a native app's `veskconfig.ts`."],
  ...
  <div class="max-w-3xl mx-auto px-4 py-14">
    ...
    <p class="text-muted">Some text here.</p>
      ...
      {
        kind: "p",
        text: "`About.vsk` is a plain statement-mode component — the exact file that also renders on the web. The compiler emits each component as a `@Composable` function plus a generated props data class. Tailwind classes become Compose `Modifier` chains and `TextStyle`s; block elements fill the parent width in column flow, mirroring the web layout. Because the input is the same IR the web compiler walks, the heading and the muted paragraph underneath keep their semantics — and their spacing — when they cross from browser to device. The Kotlin is readable, hand-writable source, which is the whole point of a compiler: your component stays one file, and the target code is an artifact you can inspect.",
      },
      {
        kind: "code",
        filename: "About.kt",
  ...
  Column(
    ...
    Text(
      ...
      { kind: "h2", text: "The native docs" },
      {
        kind: "list",
        items: [
          "Getting started — scaffold with `create-vesk-native`, install the toolchain, and run your first build.",
          ...
          "Libraries — the `libraries.json` system and the `vesk-native add/install/update/remove` verbs.",
